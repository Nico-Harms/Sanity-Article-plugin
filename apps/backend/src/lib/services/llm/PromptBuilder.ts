...
export class PromptBuilder {
  /**
   * Ensure complete modules are generated by including all fields for modules that have any enabled fields
   */
  private static ensureCompleteModules(
    detectedFields: DetectedField[]
  ): DetectedField[] {
    // For the new article schema, we don't have modules, so just return enabled fields
    return detectedFields.filter((field) => field.enabled);
  }
  ...
  static build(
    ...
    // Group fields by module for array fields to ensure complete modules are generated
    const enabledFields = PromptBuilder.ensureCompleteModules(detectedFields);
...
3. LENGTH & DEPTH:
   - Generate appropriate content for each field based on its purpose and the overall context
...
OUTPUT FORMAT:
Return ONLY a valid JSON object with EXACTLY this structure. Do not add extra characters, newlines, or formatting to field names.
...
- Use double quotes around ALL strings and field names
- Do NOT add extra characters, spaces, or newlines to field names
- Array items must be properly comma-separated
- JSON must be valid and parseable

CONTENT RULES:
- Ensure all enabled fields are populated with meaningful, expanded content
- Escape quotes with \\" and use \\n for intentional line breaks
